...
- **板の serial / USB を開くものは board 単位の lock (`tools/common/device_lock.rb`) で排他される。** 別 session が同じ板を使っていると待つ。読むだけの `shell_ok.rb` も対象。build / host テスト / QEMU / `ioreg` は対象外。詳細は [docs/spec.md](docs/spec.md) §6
