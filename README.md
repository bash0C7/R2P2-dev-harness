...
| `rake esp32:build` / `flash` / `upload` / `run` / `reboot` | 実装済み。M5Stack Chain DualKey 実機で通した (`R2P2-ESP32` の sibling checkout が要る)。**ただし `upload` / `run` の `.rb` → `.mrb` 変換を含む経路 (現在の既定) は DualKey 実機で未検証**。QEMU は PicoModem 転送を検証できない。確認したのは、ホストの mrbc で `.rb` が `.mrb` になることと、別 script (`.mrb` を直接送る) で DualKey が `.mrb` を実行できたこと (concurrency-r1-ff の実測) |
